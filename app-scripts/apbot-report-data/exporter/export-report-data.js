...
function exportReportData() {
  ...
  namedRangesToExport.forEach(rangeName => {
    try {
      ...
      const dataObjects = rangeToArrayOfObjects_(displayValues);
      allExportedData[rangeName] = dataObjects;
      Logger.log(`Successfully processed named range: "${rangeName}" (${dataObjects.length} record(s)).`);
  ...
  if (Object.keys(allExportedData).length > 0) {
    ...
    try {
      ...
      Logger.log(`Attempting to save data to User Properties. Key: 'reportData'. Length: ${dataLength} bytes.`); // Log length before saving
      ...
      if (success) {
        ...
        ui.alert('Data Export Successful', `Report data (${jsonDataString.length} bytes) has been successfully exported.`, ui.ButtonSet.OK);
        ...
        ui.alert('Data Export Failed Critically', `Critical error saving data via library. Check library logs.`, ui.ButtonSet.OK);
      }
      ...
      if (checkData && checkData.length === dataLength) {
         Logger.log(`VERIFICATION SUCCESS: Data saved to User Properties seems correct. Length: ${checkData.length}`);
      } else if (checkData) {
         Logger.log(`VERIFICATION WARNING: Data found in User Properties, but length differs! Saved: ${dataLength}, Found: ${checkData.length}. Data might be truncated.`);
      } else {
         Logger.log(`VERIFICATION FAILED: Failed to retrieve data immediately after setting it in User Properties.`);
      ...
      if (errors.length > 0) {
         ...
         const alertTitle = errors.some(e => e.includes("Critical:")) ? 'Data Export Failed Critically' : 'Data Export Partially Successful';
        ...
        ui.alert('Data Export Successful', `Report data (${dataLength} bytes) has been successfully processed and saved to User Properties.`, ui.ButtonSet.OK);
      }

    } catch (e) {
      Logger.log(`Error during JSON stringify or saving data to User Properties: ${e.message}`);
      ui.alert('Export Error', `Failed to process or save data: ${e.message}`, ui.ButtonSet.OK);
    }
  } else {
    Logger.log('Export failed: No data was successfully processed.');
    ui.alert('Export Failed', `No data could be processed. Errors:\n- ${errors.join('\n- ')}`, ui.ButtonSet.OK);
  }
}
